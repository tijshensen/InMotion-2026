      ...
      >
        <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3 shrink-0">
          <div>
            ...
            <h2 className="font-semibold">Salespage score</h2>
          </div>
          <span
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5 text-sm">
          <p className="text-xs text-slate-400">
            Tempt → Influence → Persuade → Sell, plus de 8 conversieknallers.
          </p>
          {error ? <p className="text-sm text-rose-400">{error}</p> : null}
          {!data && !busy ? (
            <p className="text-slate-500 text-sm">
              Nog niet gescoord. Klik op Bereken.
            </p>
          ) : null}
          {tips.length ? (
            <section>
        ...
        <div className="border-t border-slate-800 px-4 py-3 shrink-0">
          ...
          >
            {busy ? "Grok beoordeelt copy…" : "Bereken 8x8-score"}
          </button>
        </div>
      </aside>
    </>
